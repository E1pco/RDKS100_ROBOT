...
class GetTextFeatureClient : public rclcpp::Node
{
public:
  using GetFeatures = clip_msgs::action::GetFeatures;
  using GoalHandleGetFeatures = rclcpp_action::ClientGoalHandle<GetFeatures>;

  static std::shared_ptr<GetTextFeatureClient> GetInstance() {
    static std::shared_ptr<GetTextFeatureClient> instance = std::make_shared<GetTextFeatureClient>();
    return instance;
  }
  ...
  explicit GetTextFeatureClient(const rclcpp::NodeOptions & node_options = rclcpp::NodeOptions());
  ~GetTextFeatureClient() {
    CancelAllGoals();
  }

  int send_goal(std::vector<std::string>& texts, int timeout_seconds = 1000);
