...
namespace
{
bool rosContextOk(const rclcpp::Node::SharedPtr &node)
{
  return node && rclcpp::ok(node->get_node_base_interface()->get_context());
}

template <typename PointT>
void applyVisualizationFilters(typename pcl::PointCloud<PointT>::Ptr cloud, double voxel_size, int max_points)
{
  ...
  {
    pcl::PointCloud<PointT> limited;
    limited.reserve(max_points);
    const double step = static_cast<double>(cloud->size()) / static_cast<double>(max_points);
    for (int i = 0; i < max_points; ++i)
    {
      const auto index = std::min<size_t>(static_cast<size_t>(std::floor(i * step)), cloud->size() - 1);
      limited.points.push_back(cloud->points[index]);
    }
      ...
      extR(M3D::Identity())
{
  ...
  pcl_w_wait_pcd_pub.reset(new PointCloudXYZI());
  pcl_wait_pub.reset(new PointCloudXYZI());
  pcl_wait_pcd_pub.reset(new PointCloudXYZI());
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<int>("visualization.publish_every_n_frames", 1);
  try_declare.template operator()<bool>("visualization.dense_map_en", false);
  try_declare.template operator()<double>("visualization.voxel_size", 0.0);
  try_declare.template operator()<int>("visualization.max_points", 0);
  try_declare.template operator()<double>("visualization.publish_rate", 0.0);
  ...
  this->node->get_parameter("visualization.publish_every_n_frames", visualization_publish_every_n_frames);
  this->node->get_parameter("visualization.dense_map_en", visualization_dense_map_en);
  this->node->get_parameter("visualization.voxel_size", visualization_voxel_size);
  this->node->get_parameter("visualization.max_points", visualization_max_points);
  this->node->get_parameter("visualization.publish_rate", visualization_publish_rate);

  pub_scan_num = std::max(1, pub_scan_num);
  visualization_publish_every_n_frames = std::max(1, visualization_publish_every_n_frames);
...
{    
  ...
  PointCloudXYZI::Ptr pcdCloudBody(dense_map_en ? feats_undistort : feats_down_body);
  PointCloudXYZI::Ptr visualCloudBody(visualization_dense_map_en ? pcdCloudBody : feats_down_body);

  int visual_size = visualCloudBody->points.size();
  PointCloudXYZI::Ptr visualCloudWorld(new PointCloudXYZI(visual_size, 1));
  for (int i = 0; i < visual_size; i++)
  {
    RGBpointBodyToWorld(&visualCloudBody->points[i], &visualCloudWorld->points[i]);
  }
  *pcl_w_wait_pub = *visualCloudWorld;

  if (pcd_save_en)
  {
    int pcd_size = pcdCloudBody->points.size();
    PointCloudXYZI::Ptr pcdCloudWorld(new PointCloudXYZI(pcd_size, 1));
    for (int i = 0; i < pcd_size; i++)
    {
      RGBpointBodyToWorld(&pcdCloudBody->points[i], &pcdCloudWorld->points[i]);
    }
    *pcl_w_wait_pcd_pub = *pcdCloudWorld;
  }
  else
  {
    PointCloudXYZI().swap(*pcl_w_wait_pcd_pub);
  }
...
void LIVMapper::savePCD() 
{
  ...
  {
    std::cout << "PCD save is enabled, but no accumulated points are available yet." << std::endl;
    return;
  }
...
void LIVMapper::run(rclcpp::Node::SharedPtr &node) 
{
  ...
  while (rosContextOk(this->node)) 
  {
    rclcpp::spin_some(this->node);
    if (!rosContextOk(this->node)) break;
  ...
  std::cout << GREEN << "FAST-LIVO2 is stopping; saving PCD if enabled..." << RESET << std::endl;
  savePCD();
}
...
{
  if (!rosContextOk(this->node)) { return; }
  ...
  if (imu_prop_enable && !prop_imu_buffer.empty())
  {
    ...
    if (rosContextOk(this->node)) pubImuPropOdom->publish(imu_prop_odom);
  }
...
{
  return cv_bridge::toCvShare(img_msg, "bgr8")->image.clone();
}
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  const bool rate_limited = visualization_publish_rate > 0.0 &&
                            ...
                            now_sec - last_visualization_publish_time < 1.0 / visualization_publish_rate;
  const bool should_publish_visualization = publish_this_frame && !rate_limited && rosContextOk(this->node);
  const bool should_accumulate_pcd = pcd_save_en;

  PointCloudXYZRGB::Ptr laserCloudWorldRGB(new PointCloudXYZRGB());
  PointCloudXYZRGB::Ptr laserCloudWorldRGBSave(new PointCloudXYZRGB());
  auto project_to_rgb = [&](const PointCloudXYZI::Ptr &input, const PointCloudXYZRGB::Ptr &output)
  {
    ...
    for (size_t i = 0; i < size; i++)
    {
      ...
      pointRGB.x = input->points[i].x;
      pointRGB.y = input->points[i].y;
      pointRGB.z = input->points[i].z;

      V3D p_w(input->points[i].x, input->points[i].y, input->points[i].z);
      ...
      if (vio_manager->new_frame_->cam_->isInFrame(pc.cast<int>(), 3))
      {
        ...
        pointRGB.g = pixel[1];
        pointRGB.b = pixel[0];
        if (pf.norm() > blind_rgb_points) output->push_back(pointRGB);
      }
    }
  };
  ...
  if (img_en)
  {
    ...
    *pcl_wait_pub += *pcl_w_wait_pub;
    if (should_accumulate_pcd) *pcl_wait_pcd_pub += *pcl_w_wait_pcd_pub;
    ...
    pub_num = 1;
    if (should_publish_visualization) project_to_rgb(pcl_wait_pub, laserCloudWorldRGB);
    if (should_accumulate_pcd) project_to_rgb(pcl_wait_pcd_pub, laserCloudWorldRGBSave);
  }

  if (!should_publish_visualization && !should_accumulate_pcd)
  {
    if (img_en) PointCloudXYZI().swap(*pcl_wait_pub);
    PointCloudXYZI().swap(*pcl_w_wait_pub);
    return;
  }
  ...
  if (should_publish_visualization)
  {
    ...
    if (img_en)
    {
      ...
      PointCloudXYZRGB::Ptr laserCloudWorldRGBVis(new PointCloudXYZRGB(*laserCloudWorldRGB));
      applyVisualizationFilters<PointTypeRGB>(laserCloudWorldRGBVis, visualization_voxel_size, visualization_max_points);
      pcl::toROSMsg(*laserCloudWorldRGBVis, laserCloudmsg);
    }
    else 
    { 
      PointCloudXYZI::Ptr laserCloudWorldVis(new PointCloudXYZI(*pcl_w_wait_pub));
      applyVisualizationFilters<PointType>(laserCloudWorldVis, visualization_voxel_size, visualization_max_points);
      pcl::toROSMsg(*laserCloudWorldVis, laserCloudmsg); 
    }
    laserCloudmsg.header.stamp = this->node->get_clock()->now(); //.fromSec(last_timestamp_lidar);
    laserCloudmsg.header.frame_id = "camera_init";
    pubLaserCloudFullRes->publish(laserCloudmsg);
    last_visualization_publish_time = now_sec;
  }
  ...
  if (pcd_save_en)
  {
    ...
    if (img_en)
    {
      *pcl_wait_save += *laserCloudWorldRGBSave;
    }
    else
    {
      *pcl_wait_save_intensity += *pcl_w_wait_pcd_pub;
    }
    scan_wait_num++;
  ...
  if (img_en)
  {
    ...
    PointCloudXYZI().swap(*pcl_wait_pcd_pub);
  }
  PointCloudXYZI().swap(*pcl_w_wait_pub);
  PointCloudXYZI().swap(*pcl_w_wait_pcd_pub);
}
