...
private:
                                              ...
                                              float& threshold) const
    {
        ...
        if (label == "Annulus")
        {
            ...
            if (saturated_foreground)
            {
                ...
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s threshold %.3f is locked by saturated returns; fallback to %.3f.",
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s threshold %.3f is dominated by saturated outliers; fallback to p92 %.3f.",
                                    ...
                                    const std::string& label) const
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Unable to compute high-intensity threshold for %s.", label.c_str());
            return false;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s intensity range: %.3f - %.3f", label.c_str(), min_i, max_i);
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s Otsu: %.3f, foreground p15: %.3f, p92: %.3f, relative high: %.3f, threshold: %.3f",
                 label.c_str(), otsu_threshold, foreground_low, high_quantile, relative_high, threshold);
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s high-intensity points: %zu", label.c_str(), output->size());
        return !output->empty();
    }
                                             ...
                                             const std::string& label) const
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Empty input cloud, cannot extract high-intensity points for %s.", label.c_str());
            return false;
        }
        if (!plane_coefficients || plane_coefficients->values.size() < 4)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Invalid plane coefficients for %s.", label.c_str());
            return false;
        }
        ...
        bool interpolate_boundary) const
    {
        ...
        if (!input || input->empty())
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Empty input cloud, cannot extract ring intensity boundaries for %s.", label.c_str());
            return false;
        }
        if (!plane_coefficients || plane_coefficients->values.size() < 4)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Invalid plane coefficients for %s ring boundary extraction.", label.c_str());
            return false;
        }
        ...
        if (!computeHighIntensityThreshold(intensities, threshold, min_i, max_i,
                                           ...
                                           high_quantile, relative_high))
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Unable to compute high-intensity threshold for %s ring boundaries.", label.c_str());
            return false;
        }
        adjustHighIntensityThresholdForLabel(label, min_i, max_i, otsu_threshold,
                                             foreground_low, high_quantile,
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s ring-boundary intensity range: %.3f - %.3f", label.c_str(), min_i, max_i);
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s ring-boundary Otsu: %.3f, foreground p15: %.3f, p92: %.3f, relative high: %.3f, threshold: %.3f, jump threshold: %.3f",
                 label.c_str(), otsu_threshold, foreground_low, high_quantile,
                 relative_high, threshold, jump_threshold);
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s ring-boundary rings: %zu, valid neighbors: %zu, transitions: %zu, boundary points: %zu",
                 label.c_str(), indices_by_ring.size(), valid_neighbor_count,
                 transition_count, output->size());
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] %s ring-boundary point mode: %s",
                                                    ...
                                                    double max_error_threshold) const
    {
        ...
        double closest_score = std::numeric_limits<double>::max();
        double closest_max_error = std::numeric_limits<double>::max();
        std::vector<double> closest_measured;
        int best_idx = -1;
        ...
        for (int i = 0; i < static_cast<int>(groups.size()); ++i)
        {
            ...
            {
                closest_score = score;
                closest_max_error = max_error;
                closest_measured = measured;
            }
        ...
        if (best_idx < 0)
        {
            if (!closest_measured.empty())
            {
                std::ostringstream distances;
                distances << std::fixed << std::setprecision(3);
                for (size_t i = 0; i < closest_measured.size(); ++i)
                {
                    if (i > 0) distances << ", ";
                    distances << closest_measured[i];
                }
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"),
                            "[LiDAR] Closest candidate distances (m): [%s], target: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f], max error: %.3f m, threshold: %.3f m.",
                            distances.str().c_str(),
                            target_distances[0], target_distances[1],
                            target_distances[2], target_distances[3],
                            target_distances[4], target_distances[5],
                            closest_max_error, max_error_threshold);
            }
            return false;
        }
                            ...
                            BoardFrame& frame) const
    {
        ...
        if (normal.norm() < 1e-6)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: invalid rough plane normal.");
            return false;
        }
        normal.normalize();
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: cannot build board axes.");
            return false;
        }
        ...
        if (x_axis.norm() < 1e-6)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: invalid board x-axis.");
            return false;
        }
        x_axis.normalize();
        ...
        if (y_axis.norm() < 1e-6)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: invalid board y-axis.");
            return false;
        }
        y_axis.normalize();
                             ...
                             pcl::PointCloud<Common::Point>::Ptr board_roi_cloud) const
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI finite cloud size: %zu", finite_cloud->size());
        if (finite_cloud->size() < 1000)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: too few finite points.");
            return false;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI max-intensity voxel cloud size: %zu (leaf %.3f m)",
                 coarse_cloud->size(), auto_roi_voxel_leaf_);
        if (coarse_cloud->size() < 1000)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: too few points after max-intensity voxel sampling.");
            return false;
        }
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: too few high-intensity points.");
            return false;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI high-intensity clusters: %zu", cluster_indices.size());
        if (cluster_indices.size() < TARGET_NUM_CIRCLES)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: fewer than 4 high-intensity clusters.");
            return false;
        }
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: no high-intensity cluster set matches target geometry.");
            return false;
        }
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: cannot fit rough plane from selected reflective annuli.");
            return false;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI board cloud size: %zu", board_roi_cloud->size());
        if (board_roi_cloud->size() < 1000)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Auto ROI failed: board ROI is too small.");
            return false;
        }
                         ...
                         pcl::PointCloud<Common::Point>::Ptr board_roi_cloud) const
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Using automatic board ROI extraction.");
            if (!extractAutoBoardRoi(cloud, board_roi_cloud))
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Automatic board ROI extraction failed.");
                return false;
            }
        }
        else
        {
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Using manual pass-through board ROI extraction.");
                              ...
                              pcl::ModelCoefficients::Ptr plane_coefficients)
    {
        *filtered_cloud_ = *board_roi_cloud;
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Board ROI cloud size: %zu", filtered_cloud_->size());
        if (filtered_cloud_->size() < 1000)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Too few board ROI points.");
            return false;
        }

        if (!fitPlaneRansac(filtered_cloud_, plane_coefficients, 0.01))
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Unable to fit board plane from ROI cloud.");
            return false;
        }

        extractPlaneInliers(filtered_cloud_, plane_coefficients, plane_cloud_, 0.015);
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "Plane cloud size: %zu", plane_cloud_->size());
        if (plane_cloud_->size() < 500)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Too few board plane inliers.");
            return false;
        }
        return true;
    }
                             ...
                             PlaneAlignment& alignment)
    {
        ...
        if (normal.norm() < 1e-6)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Invalid plane normal.");
            return false;
        }
        normal.normalize();
        ...
        if (cnt == 0)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] No board plane points to align.");
            return false;
        }
                                                 ...
                                                 const PlaneAlignment& alignment)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "Extracted %zu high-intensity annulus points before voxel sampling.", edge_cloud_->size());

        if (edge_cloud_->size() < 20)
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Too few annulus points after intensity extraction.");
            return false;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Annulus points after closest-to-centroid voxel sampling: %zu (leaf %.3f m)",
                                    ...
                                    bool interpolate_boundary = true)
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Ring-boundary annulus extraction failed; fallback to high-intensity annulus points.");
            if (!extractHighIntensityPointsNearPlane(plane_cloud_, plane_coefficients,
                                                     plane_distance_threshold,
                                                     annulus_original_cloud_, "Annulus"))
            {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Ring-boundary annulus points are too sparse (%zu); fallback to high-intensity annulus points.",
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Extracted %zu high-intensity annulus points.",
                 annulus_original_cloud_->size());
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Voxel sampling made annulus cloud too sparse; keep raw annulus points for fitting.");
        }
        else
        {
            edge_cloud_.swap(annulus_cloud_downsampled);
    ...
    void clusterAnnulusCloud(std::vector<pcl::PointIndices>& cluster_indices) const
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "Number of edge clusters: %zu", cluster_indices.size());
    }
    ...
    void clusterMechanicalAnnulusBoundaryCloud(std::vector<pcl::PointIndices>& cluster_indices) const
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Mechanical annulus boundary clusters: %zu", cluster_indices.size());
    }
                                       ...
                                       pcl::PointCloud<pcl::PointXYZ>::Ptr candidate_centers) const
    {
        ...
        for (size_t i = 0; i < cluster_indices.size(); ++i)
        {
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Annulus fit failed for cluster %zu with %zu points.", i, cluster->size());
                continue;
            }

            const bool radius_ok = std::fabs(fit.radius - circle_radius_) < 0.05;
            const bool error_ok = fit.mean_abs_error < 0.03;
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Annulus cluster %zu: points=%zu, center=(%.4f, %.4f), radius=%.4f, mean abs error=%.4f",
                     i, cluster->size(), fit.x, fit.y, fit.radius, fit.mean_abs_error);

            if (!radius_ok || !error_ok)
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Reject cluster %zu: radius_ok=%d, error_ok=%d.", i, radius_ok, error_ok);
                continue;
            }
                                                 ...
                                                 pcl::PointCloud<pcl::PointXYZ>::Ptr candidate_centers) const
    {
        ...
        for (size_t i = 0; i < cluster_indices.size(); ++i)
        {
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Concentric annulus fit failed for cluster %zu with %zu boundary points.",
                         i, cluster->size());
                continue;
            }
            ...
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Concentric annulus cluster %zu: points=%zu, support=%d, center=(%.4f, %.4f), radii=(%.4f, %.4f), mean abs=%.4f, rmse=%.4f",
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Reject concentric cluster %zu: error_ok=%d, width_ok=%d.",
                         i, error_ok, width_ok);
                continue;
            }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Concentric annulus center candidates: %zu", candidate_centers->size());
    }
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr boundary_edge_cloud) const
    {
        ...
        for (size_t i = 0; i < cluster_indices.size(); ++i)
        {
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Solid boundary extraction failed for cluster %zu with %zu points.",
                         i, cluster->size());
                continue;
            }
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Solid boundary concentric fit failed for cluster %zu with %zu boundary points.",
                         i, boundary_cloud->size());
                continue;
            }

            const bool error_ok = fit.rmse < 0.025;
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Solid boundary cluster %zu: cluster=%zu, boundary=%zu, center=(%.4f, %.4f), radii=(%.4f, %.4f), mean abs=%.4f, rmse=%.4f",
                     i, cluster->size(), boundary_cloud->size(), fit.x, fit.y,
                     fit.inner_radius, fit.outer_radius, fit.mean_abs_error, fit.rmse);
            if (!error_ok)
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Reject solid boundary cluster %zu: rmse %.4f.", i, fit.rmse);
                continue;
            }
        ...
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Solid boundary concentric center candidates: %zu", candidate_centers->size());
    }
...
public:
    ...
    void detect_mech_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        auto run_mechanical_fit = [&](bool interpolate_boundary) -> MechanicalFitAttempt
        {
            ...
            if (!selectGeometryConsistentCentersByDistances(attempt.candidate_centers,
                                                            ...
                                                            0.035))
            {
                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Mechanical concentric fitting (%s boundary) did not produce 4 geometry-consistent centers from %zu candidates.",
            ...
            *(attempt.edge_cloud) = *edge_cloud_;
            ...
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Mechanical concentric fitting selected %s ring-boundary points.",
                     interpolate_boundary ? "interpolated" : "high-side");
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Mechanical %s boundary geometry max error %.2f mm.",
                     interpolate_boundary ? "interpolated" : "high-side",
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"),
                        "[LiDAR] Mechanical boundary fitting failed; fallback to high-intensity single-circle fitting.");
            if (extractAlignedHighIntensityAnnulusCloud(plane_coefficients, alignment))
            {
                ...
                {
                    RCLCPP_INFO(rclcpp::get_logger("LidarDetect"),
                                "[LiDAR] Mechanical fallback selected 4 high-intensity single-circle centers.");
                    transformCentersBackToLidar(candidate_centers, selected_indices, alignment, center_cloud);
                    return;
                }

                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"),
                            "[LiDAR] Mechanical fallback did not produce 4 geometry-consistent centers from %zu candidates.",
                            candidate_centers->size());

                pcl::PointCloud<pcl::PointXYZ>::Ptr centroid_centers(new pcl::PointCloud<pcl::PointXYZ>);
                centroid_centers->reserve(cluster_indices.size());
                for (const auto& cluster_idx : cluster_indices)
                {
                    if (cluster_idx.indices.empty()) continue;
                    ...
                    for (int idx : cluster_idx.indices)
                    {
                        const auto& p = edge_cloud_->points[idx];
                        centroid += Eigen::Vector3d(p.x, p.y, p.z);
                    }
                    centroid /= static_cast<double>(cluster_idx.indices.size());
                    ...
                    center.y = static_cast<float>(centroid.y());
                    center.z = static_cast<float>(centroid.z());
                    centroid_centers->push_back(center);
                }

                if (selectGeometryConsistentCentersByDistances(centroid_centers, selected_indices, 0.08))
                {
                    RCLCPP_INFO(rclcpp::get_logger("LidarDetect"),
                                "[LiDAR] Mechanical fallback selected 4 high-intensity cluster-centroid centers.");
                    transformCentersBackToLidar(centroid_centers, selected_indices, alignment, center_cloud);
                    return;
                }

                RCLCPP_WARN(rclcpp::get_logger("LidarDetect"),
                            "[LiDAR] Mechanical centroid fallback did not produce 4 geometry-consistent centers from %zu candidates.",
                            centroid_centers->size());
            }
            return;
        }
        ...
        *edge_cloud_ = *(best_attempt->edge_cloud);
        RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Mechanical concentric centers selected with geometry max error %.2f mm.",
    ...
    void detect_solid_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        if (!selectGeometryConsistentCenters(candidate_centers, selected_indices))
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Unable to select 4 geometry-consistent annulus centers from %zu candidates.",
                     candidate_centers->size());
            return;
        }
        ...
        if (selectGeometryConsistentCenters(boundary_candidate_centers, boundary_selected_indices))
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Solid single-circle geometry max error %.2f mm; boundary concentric %.2f mm.",
                     single_circle_geometry_error * 1000.0,
                     boundary_geometry_error * 1000.0);

            if (boundary_geometry_error < single_circle_geometry_error)
            {
                RCLCPP_INFO(rclcpp::get_logger("LidarDetect"), "[LiDAR] Use solid boundary concentric centers.");
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("LidarDetect"), "[LiDAR] Solid boundary concentric fitting did not produce 4 geometry-consistent centers from %zu candidates.",
