...
#ifndef EDGESAM_NODE_H_
#define EDGESAM_NODE_H_
...
class EdgeSamNode : public DnnNode {
 public:
  EdgeSamNode(const std::string &node_name,
                 const NodeOptions &options = NodeOptions());
  ~EdgeSamNode() override;
...
//   std::vector<float> regular_box_ = {331.2, 195.08884, 849.6, 590.0638};   // 对应 1024 * 1024 模型输入
  ...
  std::shared_ptr<EdgeSamOutputParser> output_parser_ = nullptr;
