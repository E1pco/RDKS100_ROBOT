...
int EdgeSamNode::SetNodePara() {
  ...
  model_input_height_ = tensor_properties.validShape.dimensionSize[1];
  model_input_width_ = tensor_properties.validShape.dimensionSize[2];
  ...
  int output_height = tensor_properties.validShape.dimensionSize[2];
  int output_width = tensor_properties.validShape.dimensionSize[3];
  int num_classes = tensor_properties.validShape.dimensionSize[0];
...
int EdgeSamNode::RunMulti(std::vector<std::shared_ptr<DNNTensor>>& inputs,
  ...
  encode_inputs.push_back(input_dnn_tensors[1]);
  std::vector<hbDNNTensor> encode_outputs;
  Infer(encode_inputs, encode_outputs, 0);

  int32_t const output_size{static_cast<int32_t>((input_size - 2) * 2)};
  output->output_tensors.resize(static_cast<size_t>(output_size));     

  // 2 mobile sam decode
  for (int32_t j{2}; j < input_size; j++) {
    ...
    for (int32_t i{0}; i < 2; ++i) {
      output->output_tensors[(j - 2) * 2 + i] = std::make_shared<DNNTensor>(output_dnn_tensors[i]);
    }
  }
  hbUCPFree(&(encode_outputs[0].sysMem));
...
int EdgeSamNode::PostProcess(
  ...
  for (int i = 0; i < parser_output->output_tensors.size(); i++) {
    hbUCPFree(&(parser_output->output_tensors[i]->sysMem));
  }
...
void EdgeSamNode::RosImgProcess(
  ...
  models_[0]->GetInputTensorProperties(tensor_y_properties, 0);
  hbDNNTensorProperties tensor_uv_properties;
  models_[0]->GetInputTensorProperties(tensor_uv_properties, 1);
  ...
  CacheImgType cache_img = std::make_pair(std::move(dnn_output),
               std::make_pair(std::move(tensor_y), std::move(tensor_uv)));
...
int EdgeSamNode::FeedFromLocal() {
  ...
  inputs.push_back(tensor_y);
  inputs.push_back(tensor_uv);
...
void EdgeSamNode::RunPredict() {
  while (rclcpp::ok()) {
    ...
    auto y_tensor = img_msg.second.first;
    auto uv_tensor = img_msg.second.second;
