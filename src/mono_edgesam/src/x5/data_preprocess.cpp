...
std::shared_ptr<DNNTensor> InputPreProcessor::GetNV12TensorFromNV12Img(
  ...
  hbUCPMallocCached(mem, scaled_img_height * w_stride * 3 / 2, 0);
  ...
  input_tensor->sysMem.memSize = scaled_img_height * scaled_img_width * 3 / 2;
...
std::shared_ptr<DNNTensor> InputPreProcessor::GetBoxTensor(
  ...
  auto *mem = new hbUCPSysMem;
  hbUCPMallocCached(mem, num_boxes * 4 * src_elem_size, 0);
  //内存初始化
